...
describe("match", function()
    ...
    it("or", function ()
        local ptn = r.choice("string", g.regex("[0-9]+"), fntest)
    ...
    it("delimit", function ()
        match(r.delimit(g.regex("[0-9]+"), "+"), "7", "7", 1)
        match(r.delimit(g.regex("[0-9]+"), "+"), "7+65", "7+65", 4)
        match(r.delimit(g.regex("[0-9]+"), "+"), "7+", "7", 1)
        nomatch(r.delimit(g.regex("[0-9]+"), "+"), "")
        nomatch(r.delimit(g.regex("[0-9]+"), "+"), "a+7")
        nomatch(r.delimit(g.regex("[0-9]+"), "+"), "+961")
    end)
    ...
    it("triesTimesNonGreedy", function ()
        match(r.triesTimesNonGreedy(2, 8, g.regex("."), ","), "a,a,a,aa", "a,a,", 4)
        nomatch(r.triesTimesNonGreedy(2, 8, g.regex("."), ","), "a,")
        match(r.triesTimesNonGreedy(2, 8, g.regex("."), ","), "aaaaaaaa,", "aaaaaaaa,", 9)
        nomatch(r.triesTimesNonGreedy(2, 8, g.regex("."), ","), "aaaaaaaaa,")
        nomatch(r.triesTimesNonGreedy(2, 8, g.regex("."), ","), "aaaaaaa")
        match(r.triesTimesNonGreedy(2, false, g.regex("."), ","), "a,aaa,aaa", "a,aaa,", 6)
        nomatch(r.triesTimesNonGreedy(2, false, g.regex("."), ","), "a,")
        nomatch(r.triesTimesNonGreedy(2, false, g.regex("."), ","), "aaaaaa")
        match(r.triesTimesNonGreedy(0, 8, g.regex("."), ","), "aa,aa,aa", "aa,", 3)
        match(r.triesTimesNonGreedy(0, 8, g.regex("."), ","), "aaaaaaaa,", "aaaaaaaa,", 9)
        nomatch(r.triesTimesNonGreedy(0, 8, g.regex("."), ","), "aaaaaaaaa,")
        nomatch(r.triesTimesNonGreedy(0, 8, g.regex("."), ","), "aaaaaaa")
        assert.are.equal(r.triesTimesNonGreedy(2, false, g.regex("."), ",", function(match, syn, inh) return inh + 1 end)("aaa,a,a", 1, 0).attr, 3)
    end)

    it("triesAtLeastNonGreedy", function ()
        match(r.triesAtLeastNonGreedy(2, g.regex("."), ","), "a,aaa,aaa", "a,aaa,", 6)
        nomatch(r.triesAtLeastNonGreedy(2, g.regex("."), ","), "a,")
        nomatch(r.triesAtLeastNonGreedy(2, g.regex("."), ","), "aaaaaa")
    end)

    it("triesAtMostNonGreedy", function ()
        match(r.triesAtMostNonGreedy(8, g.regex("."), ","), "aa,aa,aa", "aa,", 3)
        match(r.triesAtMostNonGreedy(8, g.regex("."), ","), "aaaaaaaa,", "aaaaaaaa,", 9)
        nomatch(r.triesAtMostNonGreedy(8, g.regex("."), ","), "aaaaaaaaa,")
        nomatch(r.triesAtMostNonGreedy(8, g.regex("."), ","), "aaaaaaa")
    end)

    it("triesZeroOrMoreNonGreedy", function ()
        match(r.triesZeroOrMoreNonGreedy(g.regex("."), ","), "a,aaa,aaa", "a,", 2)
        nomatch(r.triesZeroOrMoreNonGreedy(g.regex("."), ","), "aaaaaa")
    end)

    it("triesOneOrMoreNonGreedy", function ()
        match(r.triesOneOrMoreNonGreedy(g.regex("."), ","), ",a,aaa,aaa", ",a,", 3)
        nomatch(r.triesOneOrMoreNonGreedy(g.regex("."), ","), ",aaaaaa")
    end)
    ...
    it("lookahead", function ()
        match(r.con(r.lookahead(g.regex("[0-9]+pro")), g.regex("[0-9]+")), "765pro", "765", 3)
        match(r.con(g.regex("[0-9]+"), r.lookahead("pro")), "765pro", "765", 3)
        nomatch(r.con(r.lookahead(g.regex("[0-9]+pro")), g.regex("[0-9]+")), "765studio")
        nomatch(r.con(g.regex("[0-9]+"), r.lookahead("pro")), "765studio")
        nomatch(r.con(g.regex("[0-9]+"), r.lookahead("pro")), "765")
        match(r.con(r.lookahead(g.regex("[0-9]+pro")), g.regex("[0-9]+")), "765pro", "765", 3)
        match(r.con(g.regex("[0-9]+"), r.lookahead("pro")), "765pro", "765", 3)
        nomatch(r.con(g.regex("[0-9]+"), r.lookahead("pro")), "765studio")
    end)
    ...
    it("lookaheadNot", function ()
        match(r.con(r.lookaheadNot(g.regex("[0-9]+pro")), g.regex("[0-9]+")), "765studio", "765", 3)
        match(r.con(g.regex("[0-9]+"), r.lookaheadNot("pro")), "765studio", "765", 3)
        match(r.con(g.regex("[0-9]+"), r.lookaheadNot("pro")), "765", "765", 3)
        nomatch(r.con(r.lookaheadNot(g.regex("[0-9]+pro")), g.regex("[0-9]+")), "765pro")
        nomatch(r.con(g.regex("[0-9]+"), r.lookaheadNot("pro")), "765pro")
        match(r.con(r.lookaheadNot(g.regex("[0-9]+pro")), g.regex("[0-9]+")), "765studio", "765", 3)
    end)
...
describe("regex", function()
    ...
    it("unicode", function()
        match(g.regex("\\u0031"), "1", "1", 1)
        match(g.regex("\\u30a2"), "ア", "ア", 3)
    end)

    it("defined character set", function()
        match(g.regex("\\d"), "0", "0", 1)
        match(g.regex("\\d"), "9", "9", 1)
        nomatch(g.regex("\\d"), "/")
        nomatch(g.regex("\\d"), ":")
        match(g.regex("\\D"), "/", "/", 1)
        match(g.regex("\\D"), ":", ":", 1)
    ...
    it("grouping", function()
        ...
        matchattr(g.regex("(?:ab)+"), "ababab", "ababab", 6, checkNil)
    ...
    it("capture", function()
        ...
        matchattr(g.regex("(..)(..)e"), "abcde", "abcde", 5, checkCapture)
        matchre("(ab)+", "abbbbab", "ab", 2)
    end)

    it("starting anchor", function()
        matchre("^abc", "abc", "abc", 3)
        nomatchpos(g.regex("^abc"), "bc", 2)
    end)
